...
class Minicursos {
    ...
    public function PreencherVagaMinicurso($id) {
        $query = "UPDATE c2013_minicursos SET nvagaspreenc = nvagaspreenc + 1 WHERE id = $id";
    ...
    public function ExcluirMinicursosDoParticipante($id) {
        ...
        while ($dadosMinicursoPart = mysql_fetch_array($qryMinicursos)){
          mysql_query("DELETE FROM c2013_participantes_minicursos WHERE id_minicurso = ".$dadosMinicursoPart["id"]." AND id_participante = '$id'");
          mysql_query("UPDATE c2013_minicursos SET nvagaspreenc = nvagaspreenc - 1 WHERE id = ".$dadosMinicursoPart["id"]);
        }        
    }

    public function PesquisarPorId($id) {
        $query = "SELECT * FROM c2013_minicursos
						   WHERE id = '$id'";
        $resultado = mysql_query($query) or die("Erro: " . mysql_error());
        return $resultado;
    }
    ...
    public function PesquisarMinicursosdoParticipante($participante) {
        $query = "SELECT m.* FROM c2013_minicursos as m, c2013_participantes_minicursos as pm, c2013_inscricao as p" .
    ...
    public function Pesquisar() {
        $query = "SELECT * FROM c2013_minicursos order by nminicurso";
        $resultado = mysql_query($query) or die("Erro: " . mysql_error());
        return $resultado;
    }
